...
@section('content')
    ...
    <div class="container-xl big-padding">
        ...
        <div class="row section-title">
            <h2 class="fs-4">Unavailable Slots</h2>
        </div>
        ...
        <div class="text-white border-radius text-center mb-4 input-center shadow-md bg-dark-grey p-4 pt-5 w-50">
            ...
            <form action="{{route('unavailable-slot.store')}}" method="POST" id="AvailableSlotForm">
                @csrf
                ...
                @foreach(\App\Models\Slot::$DAYS as $key => $item)
                    <div class="row mb-3 timepicker-group timepicker-key-{{$key}}">
                        ...
                        <div class="col-lg-5 col-md-6">
                            <input type="text" autocomplete="off" class="input-center timepicker1 form-control w-75" value="{{$unavailableSlots[$key][0]['unavailable_start_time'] ?? ''}}" placeholder="HH:MM" name="start[{{$key}}]"/>
                        </div>
                        <div class="col-lg-5 col-md-6">
                            <input type="text" autocomplete="off" class="input-center timepicker2 form-control w-75" value="{{$unavailableSlots[$key][0]['unavailable_end_time'] ?? ''}}" placeholder="HH:MM" name="end[{{$key}}]"/>
                        </div>
                    </div>
                @endforeach
...
@push('scripts')
    <script type="text/javascript">
        $(function () {
            ...
            $('.timepicker-key-{{$key}}').find('input.timepicker2').timepicker('option', 'minTime', "{{$slot[0]['unavailable_start_time']}}");
            @endforeach
